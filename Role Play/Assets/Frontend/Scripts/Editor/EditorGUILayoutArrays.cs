...
public static class EditorGUILayoutArrays {
    ...
    /*
    public static List<string> StringArrayField(string label, List<string>array) {
        EditorGUILayout.Foldout(true, label);
        int newSize = array.Count;
        ...
        if (true) {
            ...
            {
                for (int i = array.Count; i > newSize; i--)
                {
                    array.RemoveAt(i-1);
                }
            }
    ...
    internal static List<string> StringArrayField(ArrayFieldSettings arrayFieldSettings, List<string> sCENE_MASTER_SERVER_PATH)
    {
